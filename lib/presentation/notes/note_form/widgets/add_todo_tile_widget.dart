  ...
  Widget build(BuildContext context) {
    return BlocConsumer<NoteFormBloc, NoteFormState>(
      ...
      listener: (context, state) {
        context.formTodos = state.note.maxListSize3.value.fold(
          (f) => listOf<TodoItemPrimitive>(),
          (todoItemList) =>
              todoItemList.map((_) => TodoItemPrimitive.fromDomain(_)),
        );
      },
      buildWhen: (p, c) => p.note.maxListSize3.isFull != c.note.maxListSize3.isFull,
      builder: (context, state) {
        return ListTile(
          enabled: !state.note.maxListSize3.isFull,
